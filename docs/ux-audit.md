...
**Retention (was §1.4):** a streak-at-risk push now fires from live streak state, alongside a new weekly recap (`src/utils/notifications.ts:120-167`). Sharing is an explicit per-log button, not a force-open (`WorkoutLogger.tsx:174-180`); the watermark uses the real domain and is now a footer call to action carrying the download link, drawn into the image and attached to the share payload (`src/utils/shareLink.ts`, `src/utils/shareCardCanvas.ts` → `drawFooter`); belt/streak/PR/fight-result cards exist alongside session cards (`ShareCard.tsx`, `CelebrationToast.tsx`, `FightBreakdown.tsx:161-162`).
