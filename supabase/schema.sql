...
------------------------------------------------------------
-- Authorization hardening (MUST remain last in this file).
--
-- WHY: the create-policy blocks earlier in this snapshot historically used
-- weaker rules (any authenticated user could change their role forever;
-- coach notes only required coach_id = auth.uid() with no active link check;
-- coach_fighter_links allowed client UPDATE of principals/status). Operators
-- were told this file is "idempotent" and safe to re-run in the SQL editor.
-- Re-running it AFTER the dedicated harden migration therefore undid
-- production hardening.
--
-- These statements are identical to
-- supabase/migrations/20260803190000_harden_authorization.sql and re-apply
-- last so a full schema.sql run always ends in the hardened state.
------------------------------------------------------------
